...
//alert("asd");
    ...
    var newurl ="http://www.greenocean.in/service/user.php"
            ...
            function gotFileWriter1(writer) 
            {
        		...
        		data[3] = "DIA:"+bpdia;
        		data[4] = "SPO2:"+spo2;
        		data[5] = "Gluco:"+glucoVal;
        		
        		var data1 = new Array();
        		
        		
        		data1[0] = "PULSE:"+cpulse;
        		data1[1] = "TEMP:"+ctemp;
        		data1[2] = "SYS:"+csys;
        		data1[3] = "DIA:"+cdia;
        		data1[4] = "SPO2:"+cspo2;
        		data1[5] = "GlucoVal:"+cgul;
				...
				writer.write("GLUCO,");
				writer.write("C_GLUCO,");
				
				writer.write("\n");
				writer.write(todayDate+",");
				writer.write(patID+",");
				
				writer.write(agender+",");
				writer.write(aage+",");
				writer.write(pulse+",");
				writer.write(cpulse+",");
				...
				writer.write(glucoVal+",");
				writer.write(cgul);
        		}
        		else
        		{
        		writer.write(previousval);
        		writer.write("\n");
        		writer.write(todayDate+",");
				writer.write(patID+",");
                ...
                /* writer.write("ASHA+ Values:");
               
                writer.write(data);
              
                writer.write("Comparision Values:");
               ...
               //var options = new FileUploadOptions();
        		...
        		options.fileName= valueId + '_' + valdate  + '_' +  p_data.substr(p_data.lastIndexOf('/')+1);
      			console.log("Renamed File Name: " + options.fileName);
	...
	ft.upload(p_data, srads+"?type=upload", DataUploadSuccess, datauploadfail, options);
    		...
    		if(patID != ""){
    		//$('#erperid').hide();
    		parentDir.getFile(device_name+".CSV", null, gotFileEntry, fails);
				//alert("srads");
				var pulse ="0";
      var temp ="0";
      var bpValue ="0";
      ...
      if(wscaltb)
       wscaltb = $("#wscalval").val(); else wscaltb = 0;
        ...
        var chdate = currhour + ":" + currminute + ":" + currsecond + " " + AP;
     	...
     	var filenme=patID+"_"+todayDate+"_Vitals.txt";
     	//var filenme="Vitals.txt";
     	//alert(filenme);			
...
if ((pulse=="0") && (temp=="0") && (bpValue=="/") && (spo2=="0"))
{
	...
	exit;
				...
				parentDir.getFile(device_name+".csv", {create: true}, gotFileEntry1, failFileEntry);
				}
				else{
				//$('#erperid').show();
...
function selectserver(tx)
    ...
    var previousval;
    var checkdata;
    function readAsTexts(file) {
        var reader = new FileReader();
        reader.onloadend = function(evt) {
            //alert("Read as text");
           previousval = evt.target.result;
           //alert(previousval);
        };
...
/*function saveFileContent1() {
	...
	{
		parentDir = dir;
	//parentDir.getFile("READVAL.CSV", null, gotFileEntry, fail);
    ...
    else
    {
        for (var i = 0; i < entries.length; i++) {
            entries[i].file(function (file) {
                ...
                if(file.name=='Vitals.csv')
                {
               // alert(device_name+"_"+file.name);
                file.name=device_name+"_"+file.name;
                
                 var target="http://greenocean.in/apollo/upload.php"; //the url to upload on server
                 }
                 else
                 {
                 var target="http://greenocean.in/apollo/upload.php";
                 }
    			var ft = new FileTransfer();
    			...
    			ft.upload(path, encodeURI(target), wins, fails, { fileName: name, fileKey: "file", mimeType:"text/plain" });
...
var ecgcurr;
	...
	var curr_month = todaysDate.getMonth();
       	...
       	var curr_hour = todaysDate.getHours();
       	var curr_min = todaysDate.getMinutes();
	var ecgdate = curr_year + '-' + curr_month + '-' + curr_date ;

        options.fileName= patID + '_' + ecgdate  + '_' +  p_data.substr(p_data.lastIndexOf('/')+1);
      	console.log("Renamed ECG File Name: " + options.fileName);
      	
        options.mimeType="text/plain";
        
        	ecgcurr=p_data.replace("ecg.txt","");
...
function ecgrenameSuccess() {
   
   //$("#stethaud").attr("src", curfile);
...
function bp_readSuccess3 (p_data)
{
		...
		//var stethdate = curr_year + '' + curr_month;
		var stethdate = curr_year + '-' + curr_month + '-' + curr_date ;	
            options.fileName= patID + '_' + stethdate  + '_' +  p_data.substr(p_data.lastIndexOf('/')+1);
            console.log("Renamed File Name: " + options.fileName);
            options.mimeType="audio/wav";
            options.chunkedMode = false;
            //var str="Visit Microsoft!";
			curr=p_data.replace("steth.wav","");
            ...
            curfile=curr+news;
            //alert(curfile);
            renameFile(old ,curr,news, renameSuccess,renameFail);
...
function renameFile(currentName, currentDir, newName, successFunction,renameFail) {
